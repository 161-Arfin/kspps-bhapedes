...
export default function HeroRecycleBinView() {
  ...
  const onOpenModal = (type: 'restore' | 'delete' | 'empty', id: number | null = null, judul: string = "") => {
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Tempat Sampah Hero</h1>
              <p className="text-sm text-gray-500 mt-0.5">Kelola hero display yang telah dihapus sementara.</p>
            </div>
          </div>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-sm p-4 flex gap-3 items-start">
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-amber-800">Penyimpanan Sementara</h4>
            <p className="text-sm text-amber-700 leading-relaxed">
              Hero display yang berada di tempat sampah dapat dipulihkan kembali ke daftar utama atau dihapus secara <span className="font-bold">permanen.</span>
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 overflow-hidden">
            ...
            <table className="w-full min-w-[800px] text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-200">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Waktu Hapus</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {isLoading ? (
                  ...
                  dataSampah.map((hero, index) => (
                    <tr key={hero.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="relative w-20 h-12 rounded bg-gray-100 overflow-hidden border border-gray-200 opacity-60">
                            {hero.gambar && <img src={hero.gambar} className="w-full h-full object-cover grayscale" alt="" />}
                          </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center">
                          <button
                            onClick={() => onOpenModal('restore', hero.id, hero.judul)}
                          ...
                          <button
                            onClick={() => onOpenModal('delete', hero.id, hero.judul)}
          ...
          <div className="px-6 py-4 bg-gray-50/30 border-t border-gray-100 flex items-center justify-between">
            <span className="text-xs text-gray-400">
              {isLoading ? '...' : `Menampilkan ${dataSampah.length} data terhapus`}
            </span>
          </div>
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        title={
          modalConfig.type === 'restore' ? "Pulihkan Hero?" :
            modalConfig.type === 'empty' ? "Kosongkan Sampah?" :
              "Hapus Permanen?"
        }
        message={
          modalConfig.type === 'restore' ? `Hero "${modalConfig.judul}" akan dikembalikan ke daftar utama.` :
            modalConfig.type === 'empty' ? "Semua data hero di tempat sampah akan dihapus selamanya dan tidak bisa dikembalikan." :
              `Hapus hero "${modalConfig.judul}" selamanya? Tindakan ini tidak dapat dibatalkan.`
        }
        confirmText={
          modalConfig.type === 'restore' ? "Ya, Pulihkan" :
            modalConfig.type === 'empty' ? "Ya, Kosongkan" :
              "Hapus Selamanya"
        }
